...
def raise_exception_handler(error_message: str):
        ...
        if "http" in list_of_streamers[0]:
            ...
            while error_count < 4:
                try:
                    ...
                    logger.error('was unable to get list of streamers trough request with response: %s with exception: %s', get_streamers_trough_request_response, e)
                    discord_remote_log("Goinglivebot","red",f"was unable to get list of streamers trough request with response: {get_streamers_trough_request_response} with exception: {e}",False)
                    send_gotify_notification("goinglive",f"was unable to get list of streamers trough request with response: {get_streamers_trough_request_response} with exception: {e}","5")
            if error_count == 4:
