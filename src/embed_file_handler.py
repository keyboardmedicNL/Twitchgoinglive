...
def save_message_id_to_file(name: str , i: int, message_id: str ,user_login: str, embed_color: str, user_name: str):
    i = str(i)

    with open(f"config/embeds/{i}/{name}.txt", 'w') as file_to_save_message_id_to:
...
def read_message_id_from_file(name: str, i: int) -> tuple[str, str, str, str]:
    i = str(i)

    with open(f"config/embeds/{i}/{name}.txt", 'r') as file_to_read_from:
...
def remove_message_id_file(name: str, i: int):
    i = str(i)
    os.remove(f"config/embeds/{i}/{name}.txt")
